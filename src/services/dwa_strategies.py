...
def balanced_strategy(provider: ProviderMetrics) -> float:
    ...
    cost_score = max(0.1, 2.0 - provider.cost) if provider.cost > 0 else 1.0  # Lower cost = higher score
    speed_score = max(0.1, 3.0 - provider.speed) if provider.speed > 0 else 1.0  # Lower time = higher score
...
def adaptive_learning_strategy(provider: ProviderMetrics) -> float:
    ...
    if len(provider.accuracy_history) > 1:
        accuracy_variance = sum([(x - provider.accuracy) ** 2 for x in provider.accuracy_history]) / len(provider.accuracy_history)
        consistency_score = max(0.1, 1.0 - accuracy_variance)  # Lower variance = higher consistency
    else:
        consistency_score = 0.5
    
    if len(provider.speed_history) > 1:
        speed_variance = sum([(x - provider.speed) ** 2 for x in provider.speed_history]) / len(provider.speed_history)
